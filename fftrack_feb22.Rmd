...
runlist <- tibble(
  ...
  "Q10_RH" = rlnorm(100, lognorm(2, 1.0)[1], lognorm(2, 1.0)[2]),
  "ECS" = rlnorm(100, lognorm(3, 1.0)[1], lognorm(3, 1.0)[2])
)
...
# Will need to remove timer
# Figure out try() or tryCatch()
system.time({
  for(row in seq_len(nrow(runlist))) {
    output[[runlist$run_number[row]]] <- try(run_hector(runlist[row,][-1], core))
  }  
})
...
one <- ggplot(single, aes(BETA, Q10_RH, z = source_fraction, color = source_fraction)) +
  geom_point() + 
...
two <- ggplot(single, aes(BETA, ECS, color = source_fraction)) +
  geom_point() +
  scale_color_viridis_c() +
  labs(x = "beta",
       y = "ECS",
...
three <- ggplot(single, aes(ECS, Q10_RH, color = source_fraction)) +
  geom_point() +
  scale_color_viridis_c() +
  labs(x = "ECS",
       y = "Q10",
...
test <- output %>%
  ...
  mutate(destination_fraction = destination_percent / 100)

# Pie chart of quantity of carbon in atmosphere by source
# Needs work
ggplot(test, aes(x = "", y = source_quantity, fill = source_name)) +
  geom_bar(stat = "identity", width = 1) +
  scale_fill_viridis_d() +
  coord_polar("y", start=0) +
  labs(title = "Amount of carbon in the atmosphere by source") +
  theme_void()

# Destination fraction plots

# Just looking at five runs
test15 <- filter(test, run_number %in% c(1:5))

ggplot(test15, aes(year, source_quantity, 
                 fill = source_name)) +
  geom_area() +
  scale_fill_viridis_d() +
  facet_wrap(~run_number)

ggplot(test15, aes(year, destination_fraction, 
                 fill = source_name)) +
  geom_area() +
  scale_fill_viridis_d() +
  facet_wrap(~run_number)
