...
# We do replace=TRUE in case there's fewer than 100 runs available
...
The following sections contain tentative figures in the order they would appear in a manuscript.
...
temp_plot <- model_output_filtered %>%
  filter(variable == "global_tas",
         year >= 2015 & year <= 2100) %>% 
...
co2_plot <- plot_data_co2 %>% 
  ...
  labs(x = "Year",
       y = "CO2 (ppm)",
       color = "Source",
       fill = "Source") ; co2_plot

cmip_plots <- grid.arrange(temp_plot, co2_plot)
ggsave(plot = cmip_plots, "./figures/fig1.png", height = 6, width = 9, units = "in")
...
source_pool_plot <- atmosphere_pool %>%
  ...
  stat_summary(color = "black",
               ...
               geom = "line",
               group = "run_number",
               size = 0.7) ; source_pool_plot

ggsave(plot = source_pool_plot, "./figures/fig2.png", height = 6, width = 9, units = "in")
```
...
source_fraction_plot <- ggplot(ff, aes(year, color = ssp, fill = ssp)) +
  ...
  labs(x = "Year",
       ...
       color = "Scenario",
       fill = "Scenario") ; source_fraction_plot

ggsave(plot = source_fraction_plot, "./figures/fig3.png", height = 6, width = 9, units = "in")
...
destination_plot <- ffi_destinations %>%
  filter(ssp == MAIN_SCENARIO) %>%
  ...
  labs(x = "Year",
       ...
       fill = "Destination") ; destination_plot
...
ggsave(plot = destination_plot, "./figures/fig4.png", height = 6, width = 9, units = "in")
...
relaimpo_plot <-relimp_out %>%
  ...
  labs(x = "Year",
       ...
       fill = "Parameter") +
  theme(axis.text.x = element_text(angle = 90)) ; relaimpo_plot

ggsave(plot = relaimpo_plot, "./figures/fig5.png", height = 6, width = 9, units = "in")
...
tracking_years_plot <- all_years_tracking %>%
  filter(source_name == "earth_c", pool_name != "earth_c") %>%
  mutate(source_quantity = pool_value * source_fraction) %>%
  group_by(year) %>%
  mutate(destination_fraction = source_quantity / sum(source_quantity))%>% 
  ...
  labs(x = "Year",
       ...
       fill = "Parameter") +
  scale_fill_viridis_d(breaks = names(o_pool_labels),
                       labels = o_pool_labels) ; tracking_years_plot

ggsave(plot = tracking_years_plot, "./figures/fig6.png", height = 6, width = 9, units = "in")
...
af_plot <- ggplot(plot_data, aes(x = year, fill = def)) +
  ...
  labs(x = "Year",
      ...
      color = "Computation",
      fill = "Computation") +
  theme(axis.text.x = element_text(angle = 90)) ; af_plot

ggsave(plot = af_plot, "./figures/fig7.png", height = 6, width = 9, units = "in")
...
distribution_plot <- runlist %>% 
  ...
  labs(x = NULL, y = NULL) ; distribution_plot

ggsave(plot = distribution_plot, "./figures/figs1.png", height = 6, width = 9, units = "in")
...
# Plot
diagnostic_plot <- left_join(hector_temp_slice, run_fail_summary, by = c("run_number", "ssp")) %>%
  ...
  labs(x = "Year",
       y = expression(degree*C),
       color = "Fraction of failed runs") ; diagnostic_plot

ggsave(plot = diagnostic_plot, "./figures/figs3.png", height = 6, width = 9, units = "in")
...
ggpairs_plot <- ggpairs(ffi_atm_2100,
        columns = 10:(10 + (n-1)),
  ...
  labs(color = "Source Fraction") ; ggpairs_plot

ggsave(plot = ggpairs_plot, "./figures/figs4.png", height = 6, width = 9, units = "in")
