...
``` {r ca-comp, fig.cap = " Hector CO2 data in 1971 and 2021 compared to observational data from NOAA"}
noaa <- read.csv("noaa_co2_data.csv")
noaa71 <- noaa %>% filter(year == 1971)
noaa21 <- noaa %>% filter(year == 2021)

hector_co2_71 <- model_output %>%
  filter(variable == "Ca",
         year == 1971) %>%
  select(run_number, year, value)

hector_co2_21 <- model_output %>%
  filter(variable == "Ca",
         year == 2021) %>%
  select(run_number, year, value)
...
hector_co2_71 %>%
  ...
  geom_vline(aes(xintercept = noaa71$mean), color = "red") +
  labs(x = "Atmospheric CO2 (ppm)",
       y = "Count",
       title = "Hector atmospheric CO2 output versus NOAA observational data (1971)",
       color = "NOAA")

hector_co2_21 %>%
  ggplot(aes(value)) + 
  geom_histogram(bins = 35) +
  geom_vline(aes(xintercept = noaa21$mean), color = "red")  +
  labs(x = "Atmospheric CO2 (ppm)",
       y = "Count",
       title = "Hector atmospheric CO2 output versus NOAA observational data (2021)")
...
set_tracking <- function(c, date){
  ...
  reset(c)
  # Side note - run(c, runtodate = (date + 50)) returned an error and wouldn't run
  run(c, (date + 50))
...
# Calculate true AF for the first year after tracking only
true_dates <- dates + 1

# Find cumulative emissions
emissions_yr1 <- all_years_tracking %>%
  filter(pool_name == "earth_c",
         source_name == "earth_c",
         year %in% c(dates, true_dates)) %>%
  arrange(year) %>%
  # the NA at the end here is so that cumulative emissions 'line up' correctly
  # with the source_quantity (earth-origin atmosphere C) calculated below
  mutate(emissions = -c(diff(pool_value), NA),
         ...
         cumulative_emissions = cumsum(emissions)) %>%
  select(year, cumulative_emissions) %>%
  slice(-c(3,6,9,12,15,18,21,24,27,30))
  
# Compute the fraction of fossil fuel emissions residing in the atmosphere
emissions_fraction_yr1 <- all_years_tracking %>%
  filter(pool_name == "atmos_c", 
         source_name == "earth_c",
         year %in% emissions_yr1$year) %>%
  # How much earth_c is in atmos_c? 
  mutate(source_quantity = pool_value * source_fraction) %>%
  left_join(emissions_yr1, by = "year") %>%
  select(year, source_quantity, cumulative_emissions) %>%
  mutate(c_emissions_fraction = source_quantity / cumulative_emissions)

year_one <- filter(emissions_fraction_yr1, year %in% true_dates)
