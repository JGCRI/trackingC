...
# Filter for co2 and calculate summary metrics
...
cmip_hist_repeat <- list()
for(n in names(SSP_files)){
  cmip_hist_repeat[[n]] <- cmip_co2_hist %>% 
    mutate(ssp = n)
}
cmip_hist_repeat <- bind_rows(cmip_hist_repeat)

cmip_final <- bind_rows(cmip_hist_repeat, cmip_scenarios) %>% arrange(ssp, year)
...
carbon_h <- model_output %>%
  ...
  left_join(cmip_bounds_h, by = c("year", "ssp")) %>%
...
model_output_filtered_1 <- model_output %>%
  left_join(run_fail_summary_h, by = c("ssp", "run_number")) %>% 
  filter(fraction_fail < 0.3) %>% 
...
# Identify unrealistic runs - for each run, if more than 50% of values are
# outside of the min/max of the CMIP data, the run is nixed
cmip_bounds_ssp <- cmip_final %>% 
  filter(year %in% 2015:2100) %>%
  group_by(ssp) %>%
  select(year, minimum, maximum) %>%
  # convert from mol mol-1 to ppmv
  mutate(minimum = minimum * 1e6,
         maximum = maximum * 1e6) %>%
  distinct() %>%
  ungroup()
...
# min or max of CMIP
carbon_ssp <- model_output_filtered_1 %>%
  filter(variable == "CO2_concentration",
         year >= 2015 & year <= 2100) %>%
  select(run_number, ssp, year, value) %>%
  left_join(cmip_bounds_ssp, by = c("year", "ssp")) %>%
  mutate(fail = value < minimum | value > maximum)

# Summarize
run_fail_summary_ssp <- carbon_ssp %>%
  group_by(ssp, run_number) %>%
  summarize(fraction_fail = sum(fail) / n())

# Filter out 'unrealistic' runs (identified above)
model_output_filtered_2 <- model_output_filtered_1 %>%
  left_join(run_fail_summary_ssp, by = c("ssp", "run_number")) %>% 
...
# What is going on
carbon_ssp %>% 
  ggplot(aes(x = year, group = run_number)) + 
  geom_line(aes(y = value)) + 
  geom_line(aes(y = minimum, color = "CMIP")) + 
  geom_line(aes(y = maximum, color = "CMIP")) + 
  facet_wrap(~ssp)
...
c_hist <- carbon %>% filter(year < 2015)

# Summarize
run_fail_summary <- c_hist %>%
  group_by(ssp, run_number) %>%
  summarize(fraction_fail = sum(fail) / n(),
            .groups = "keep")

# Tighter constraint for historical data
carbon_filter_h <- carbon %>%
  left_join(run_fail_summary, by = c("ssp", "run_number")) %>% 
  filter(fraction_fail < 0.3) %>%
  summarize(pass = unique(run_number))

# Now, filter CO2 data only for the runs that pass
carbon_pass_h <- carbon %>%
  filter(run_number %in% carbon_filter_h$pass)
...
cmip_t_sce <- bind_rows(cmip_tgav_sce)

# Filter for Tgav and calculate summary metrics
cmip_t_scenarios <- cmip_t_sce %>%
  rename(ssp = experiment) %>%
...
# Need to append the historical values to each SSP
# Add SSP column to historical data, repeat for all scenarios
cmip_hist_repeat_tgav <- list()
for(n in names(SSP_files)){
  cmip_hist_repeat_tgav[[n]] <- cmip_tgav_hist %>% 
    mutate(ssp = n)
}
cmip_hist_repeat_t  <- bind_rows(cmip_hist_repeat_tgav)
...
# Get temperature data from Hector, add "fail" column if values are outside the
# min or max of CMIP
tgav_h <- model_output_filtered_2 %>%
...
# Filter out 'unrealistic' runs (identified above)
model_output_filtered_3 <- model_output_filtered_2 %>%
...
# Note that the tracking output needs to be filtered as well
trk_output_filtered_3 <- trk_output_filtered_2 %>%
  left_join(run_fail_summary, by = c("ssp", "run_number")) %>% 
  filter(fraction_fail < 0.5)
...
# Get temperature data from Hector, add "fail" column if values are outside the
# min or max of CMIP
tgav_h_ssp <- model_output_filtered_3 %>%
  filter(variable == "global_tas",
         year >= 2015 & year <= 2100) %>%
  select(run_number, ssp, year, value) %>%
  left_join(cmip_bounds_t, by = c("year", "ssp")) %>%
...
# Filter out 'unrealistic' runs (identified above)
model_output_final <- model_output_filtered_3 %>%
  left_join(run_fail_summary_tgav_ssp, by = c("ssp", "run_number")) %>% 
  filter(fraction_fail < 0.5)

rm(model_output_filtered_3)

# Note that the tracking output needs to be filtered as well
trk_output_final <- trk_output_filtered_3 %>%
  left_join(run_fail_summary_ssp, by = c("ssp", "run_number")) %>% 
  filter(fraction_fail < 0.5)

rm(trk_output_filtered_3)
...
hector_temp_slice <- left_join(hector_temp_runs, temperature, by = c("run_number", "ssp"))
