...
results <- list()
for(n in seq_len(length(out))){
  results[[n]] <- bind_rows(out[[n]]$results)
}
...
cmip <- read.csv("CMIP6_annual_tas_global.csv") %>%
  # filter by experiment, stop x axis at 2300
...
ggplot(temp, aes(year, fill = source)) +
