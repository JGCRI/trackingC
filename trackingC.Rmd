...
trk_output_final <- fread("./output_files/trk_final.csv")

dest_output <- fread("./output_files/dest_output.csv")
dest_runs <- trk_output_final$run_number
...
rm(list = c("cmip_co2", "cmip_co2_hist", "cmip_bounds_h", "cmip_bounds_t", "cmip_hist_repeat",
     ...
     "cmip_tgav_hist", "cmip_tgav_sce", "tgav_h", "tgav_h_ssp", "hector_temp_slice",
     "hector_temp_runs", "hector_co2", "carbon_h"))
```
...
atm_plot <- atm_summary %>%
  ...
  labs(x = "Year",
       y = "Source fraction of CO2 in atmosphere") ; atm_plot
  scale_x_continuous(expand = c(0, 0)) ; atm_plot
 

percent_summary <- atmosphere_pool %>%
  filter(year %in% c(2015, 2100, 2300),
         source_name == "earth_c") %>%
  group_by(ssp, year) %>%
...
average_summary <- dest_average %>% 
  ...
  mutate(dest_avg = sum(dest_avg))
  distinct(pool_name)
  summarize(avg = mean(dest_avg))
...
# Process data
dest <- dest_output %>% 
  filter(run_number %in% dest_runs) %>%
...
rm(list = c("dest_plot", "all_years_tracking", "dest", "dest_e2a_minimal", 
...
af_plot_data <- bind_rows(emissions_fraction, classic_af) %>%
  ...
  filter(year %in% 1910:2100) # past 2200, data gets weird
