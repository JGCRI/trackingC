...
for(n in names(SSP_files)){
  cmip_sce[[n]] <- cmip_raw %>% 
    filter(experiment == n | experiment == "historical")
}  
...
cmip_minmax_bounds <- cmip_scenarios %>% 
  group_by(ssp) %>%
  select(year, minimum, maximum) %>%
  ungroup()

hist <- cmip_minmax_bounds %>%
  filter(ssp == "historical") %>%
  select(-ssp)

cmip_bounds <- cmip_minmax_bounds %>% filter(ssp != "historical", year > 2016)
...
temperature <- bind_rows(hector_temp, hist_temp) %>% arrange(ssp, year)

# hector_test <- bind_rows(hector_hist, hector_temp)
...
cmip_plot <- cmip_scenarios %>% filter(ssp != "historical")
...
model_output_filtered %>%
  ...
  facet_wrap(~ssp, scales = "free",
             #labeller = labeller(ssp = c("Historical", ssp_labels)),
             ncol = 3) +
