...
diffusion_vals <- rnorm(nrow(runlist), mean = 1.18, sd = 0.118)
...
run_hector <- function(pdata, c, sce) {
  ...
  out$results <- fetchvars(c, OUTPUT_YEARS, 
                           c("CO2_concentration", "RF_tot",
                             "RF_CO2", "global_tas", "luc_emissions", "NBP")) %>% 
    # remove unneeded rows to shrink outputs
...
```{r output-files}
fwrite(runlist, "../output/runlist_test.csv", row.names = FALSE)
fwrite(model_output, "../output/model_output_test.csv", row.names = FALSE)
fwrite(trk_output, "../output/trk_output_test.csv", row.names = FALSE)
fwrite(dest_output, "../output/dest_output_test.csv", row.names = FALSE)
```
