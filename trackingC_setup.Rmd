...
# Create a runlist for each SSP
# First, create scenario identifier and run_number columns using lapply
# (across the scenarios) and rep() to generate as many rows as 
# called for by the corresponding entry in SSP_runs
runlist_list <- lapply(seq_along(SSP_runs), 
                       function(x) tibble(ssp = rep(names(SSP_runs)[x],
                                                    SSP_runs[x])))
runlist <- bind_rows(runlist_list)
runlist$run_number = seq_len(nrow(runlist))
